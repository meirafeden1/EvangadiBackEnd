...
router.get("/:question_id", async (req, res) => {
  ...
  try {
    const [answers] = await db.query(
      "SELECT a.answer_id, a.content, u.username AS user_name, a.created_at FROM answers a JOIN users u ON a.user_id = u.user_id WHERE a.question_id = ? ORDER BY a.created_at ASC",
      [question_id]
    );
    if (answers.length === 0) {
      return res
...
router.post("/", async (req, res) => {
  const { questionid, answer, user_id } = req.body;
  if (!questionid || !answer || !user_id) {
    return res
      .status(400)
